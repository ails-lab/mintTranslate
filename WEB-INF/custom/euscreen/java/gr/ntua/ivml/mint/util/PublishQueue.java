...
public class PublishQueue {
	...
	// static final Logger pubLog = Logger.getLogger( "gr.ntua.ivml.mint.PubLog" );
	...
	public static void queueItem( Item item, boolean remove ) {
		...
		// queue
		queue.add( qi );
		log.info("Queued " + item.getPersistentId() + " for " + ( remove?"remove":"update" ) );
	}
	...
	public static void workQueue() {
		...
		while( true ) {
			try {
				...
				log.error( "There was an interrupted exception", ie);
			}
		}
	}
	...
	public static boolean sendItem( Item i, String euscreenId ) {
		...
		try {
			...
			log.info( "Posting " + euscreenId + " for " + (i!=null?"update":"remove"));
			Request req = Request.Post(Config.get( "portal.indexurl" ))
			.body(new UrlEncodedFormEntity(params, "UTF-8"))
			.socketTimeout(5000);
			// check for proxy
			String proxyHost = (String ) System.getProperties().get("http.proxyHost" );
			if( proxyHost != null ) {
				String proxyPort = (String ) System.getProperties().get("http.proxyPort" );
				if( proxyPort != null ) {
					req = req.viaProxy(new HttpHost( proxyHost, Integer.parseInt(proxyPort)));
				} else {
					req = req.viaProxy(new HttpHost( proxyHost ));	
				}
			}
