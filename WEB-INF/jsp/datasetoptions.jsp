	...
	<!-- root is defined so higher option is show items -->
		<s:if test="current.isRootDefined()==true && current.isItemized()==true">
		 	<%if(user.hasRight(User.SUPER_USER) || (!Config.getBoolean("disablePublish") && user.hasRight(User.PUBLISH))){ %>
		    	<s:if test="!current.isTransfOrAnnotatedWithTransfParent()"> 
		    		<!-- checking if this is an upload or transformation -->
		    		...
		    		<s:else>
			    		...
			    		<s:if test="du.isDirectlyPublishable()==true && current.getOrg().isPublishAllowed()">
				     		<div title="Direct Publish" data-load='{"kConnector":"html.page", "url":"XSLselection?uploadId=<s:property value='uploadId'/>&orgId=<s:property value='organizationId'/>&userId=<s:property value='userId'/>&direct=<s:property value='true'/>", "kTitle":"Dataset Publish" }' class="items navigable">
				     			<div class="label">Publish through Operation Direct (experimental)</div>
				      			<div class="tail"></div>
				      		</div>   
					    </s:if>
...
<s:if test="current.isProcessing()==false">
	<s:if test="current.isRootDefined()==true && current.isItemized()==true">
		...
		<%if(user.hasRight(User.MODIFY_DATA)){ %>
		  	...
		  	<!--  access mappings --> 
		   	...
		   	<s:if test="du.getSchema() != null && !current.isPublished()" >
	 			<% if(user.hasRight(User.SUPER_USER) || !Config.getBoolean("ui.hide.annotator")) { %>			   		
		  			<!--  access annotations -->   
					<div title="Annotate" onclick="$K.kaiten('load', {kConnector: 'html.page', url: 'Annotator.action?uploadId=<s:property value='uploadId'/>', kTitle:'Annotator'});" class="items navigable">
			...
			<% if(user.hasRight(User.SUPER_USER) || !Config.getBoolean("ui.hide.mapping")) { %>
			<!--  access mappings -->
			<div title="Enrich" data-load='{"kConnector":"html.page", "url":"Enrich_input.action?uploadId=<s:property value='uploadId'/>&orgId=<s:property value='organizationId'/>&userId=<s:property value='userId'/>", "kTitle":"Enrich" }' class="items navigable">
		...
		<%} %>	  
	</s:if>  <!-- if( root defined and itemized), end  -->
		...
		<% if(((String) request.getAttribute("datasetType")).equals("AnnotatedDataset")) { %>
			...
			<s:if test="current.isReadOnly()==false">
				<div title="Undo <s:property value="datasetType"/>"  class="items navigable"
				   onclick="var cp=$($(this).closest('div[id^=kp]')); ajaxUndoAnnotations('${current.name}', ${uploadId}, ${user.dbID},${organizationId},cp);">
	...
	<s:if test="!current.isTransfOrAnnotatedWithTransfParent() && transformations.size>0">
		...
		<s:iterator var="trans" value="transformations">
			...
			%>
	   		...
	   		<div title="<s:property value="name"/>" 
	 			...
	 			$K.kaiten('load',{kConnector:'html.page', url:'DatasetOptions_input.action?uploadId=<s:property value="dbID"/>', kTitle:'Dataset Options' });"
	 		 	class="items navigable">
	 				
				<div class="label" style="width: 80%; margin-left: 10px;">						
					<s:property value="name"/>&nbsp;&nbsp;&nbsp;<font size="0.8em"><s:property value="created"/></font>
				</div>
				<div class="info">
...
</s:if> <!--  end of current processing false -->
<div id="hello_react" />

</div></div> <!--  body panel and block nav div -->
   ...
   if(parent.getDerivedTransfLeaves()!=null){
	...
	for(Dataset d: parent.getDerivedTransfLeaves() ) {
		 //if(d instanceof gr.ntua.ivml.mint.persistent.Transformation ){
			 ...
			 html.append("<div title=\""+d.getName()+"\" onclick=\"var cp=$($(this).closest('div[id^=kp]'));$(cp).find('div.k-active').removeClass('k-active');$(this).toggleClass('k-active');$K.kaiten('removeChildren',cp, false);"+
			 				"$K.kaiten('load',{kConnector:'html.page', url:'DatasetOptions_input.action?uploadId="+d.getDbID()+"', kTitle:'Dataset Options' });\""
			 			    +" class=\"items navigable\">");
...
jQuery(document).ready(function(){
	ajaxProjectLabelDraw(${uploadId},${userId},${organizationId});
	$(".accordion").accordion({ header: "h3", autoHeight: false, collapsible: true, active: false});
	    ...
	    let domContainer = document.querySelector('#hello_react');
