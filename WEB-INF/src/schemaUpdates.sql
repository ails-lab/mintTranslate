...
-- annotations 
select execute ( $$ CREATE TABLE annotation_set
  annotation_set_id int primary key NOT NULL,
  creation_date timestamp without time zone,
  name text,
  project_ids smallint[] default '{}' NOT NULL,
  organization_id integer references organization,
  target_dataset_id integer references dataset,
  target_original_dataset_id integer references dataset,
  payload bytea,
  
  -- {"format": "gzip-json-ld"} is the default here, we may get more formats or more fields
   
  config_json text,
  
  CONSTRAINT annotation_set_organization_fk FOREIGN KEY (organization_id)
    REFERENCES organization (organization_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
    NOT VALID,

  CONSTRAINT annotation_set_target_dataset_fk FOREIGN KEY (target_dataset_id)
    REFERENCES dataset(dataset_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE SET NULL
    NOT VALID,

  CONSTRAINT annotation_set_target_original_dataset_fk FOREIGN KEY (target_original_dataset_id)
    REFERENCES dataset(dataset_id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE SET NULL
    NOT VALID
...
$$ ) where NOT table_exists( 'annotation_set');


select execute( $$ create sequence seq_annotation_set_id start with 1000 $$ )
where not sequence_exists( 'seq_annotation_set_id');
