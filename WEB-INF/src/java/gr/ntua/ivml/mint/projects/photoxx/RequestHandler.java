package gr.ntua.ivml.mint.projects.photoxx;
...
public class RequestHandler {
	...
	private static Dataset accessAllowed( HttpServletRequest request, HttpServletResponse response, ErrorCondition err ) throws Exception {
		...
		Project p = err.grab(()-> DB.getProjectDAO().findByName("photoxx"), "'photoxx' project not present in db" );
		err.check(() -> p.hasProject(ds.getOrigin()), "Dataset not in photoxx" );
		...
		try {
			...
			err.onValid(() -> {
				Optional<PublicationRecord> oPr = DB.getPublicationRecordDAO().findByOriginalDataset( ds.getOrigin()) 
						.stream()
						.filter( pr -> pr.getTarget().equals( "photoxx"))
						.findAny();
				if( oPr.isPresent() )
	...
	public static void oaiPublishGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			...
			err.check(()-> PublicationHelper.isSchemaAndHasValidItems(ds, Config.get( "photoxx.oai.schema")),
			...
			err.onValid( ()-> {
				...
				Runnable r = () -> {
					try {
						...
						PublicationHelper.oaiPublish(myDs, "photoxx", pubUser
							, Config.get( "photo.queue.exchange")
							, Config.get( "photo.queue.routingKey")
							, Config.get( "photo.oai.server" )
							, Integer.parseInt( Config.get( "photo.oai.port" ))
							/* interceptor to mess with ItemMessages ds */ 
							, modifyItem( ) 
						);
				...
				Queues.queue( r, "db");
				ds.logEvent( "Publication to photoxx OAI queued.");
	...
	private static Interceptor<ItemMessage, ItemMessage> modifyItem( ) {
		// Build the dom
	...
	public static ArrayNode datasetPublishOptions( Dataset ds, User user ) {
		...
		if( !projects.contains("photoxx")) return result;
		
		
		List<PublicationRecord> pubs = DB.getPublicationRecordDAO().findByOriginalDataset( ds.getOrigin());
		Optional<PublicationRecord> relevantPub = pubs.stream().filter( pr -> pr.getTarget().equals( "photoxx")).findAny();
		...
		if( relevantPub.isPresent()) {
			...
			case PublicationRecord.PUBLICATION_OK:
				option.label = "Unpublish from PhotographyXX OAI";
				option.url = "api/photoxx/oaiUnpublish/"+relevantPub.get().getPublishedDataset().getDbID();
				break;
			case PublicationRecord.PUBLICATION_RUNNING:
				option.label = "PhotographyXX OAI publication running";
				option.inProgress = true;
				option.url = null;
				break;
			case PublicationRecord.PUBLICATION_FAILED:
				option.label = "Clean failed PhotographyXX OAI publication";
				option.url = "api/photoxx/oaiClean/"+relevantPub.get().getPublishedDataset().getDbID();
				break;
			}
			result.add( Jackson.om().valueToTree(option));
		} else {
			// is there a unique suitable dataset in the tree?
			Optional<Dataset> oDs = PublicationHelper.uniqueSuitableDataset(ds, Config.get( "photoxx.oai.schema"));
			if( oDs.isPresent() ) {
				option.label = "Publish to PhotographyXX OAI";
				option.url = "api/photoxx/oaiPublish/"+oDs.get().getDbID();				
				result.add( Jackson.om().valueToTree(option));
			}
		}
