 ...
 * 
 * 10 years later and with more experience, this design is crap. The reflection doesn't give any help
 * in finding the  method. Newer checks will be more explicitly typed
...
public class AccessAuthenticationLogic {
	...
	public static boolean canUserWrite( User user, Organization org ) {
		return //iSuper
				( user.getRights() == User.SUPER_USER) ||
				((user.getRights() > 0 ) &&
					user.isAccessibleOrganization(org));
	}
	
	public static boolean canUserRead( User user, SecurityEnabled se) {
		return //iSuper
				( user.getRights() == User.SUPER_USER) ||
				((user.getRights() > 0 ) &&
				(
					// org is in user or org is suborg from user org
					( user.isAccessibleOrganization(se.getOrganization())) ||
					// org has signed up for project where user is admin
					!Project.sharedIds(user.getProjectIds(),se.getProjectIds()).isEmpty()));
	}
