...
public class Transformation extends Dataset implements Lockable {
	...
	private String jsonEnrichmentParameters;
	...
	public static Transformation fromDataset( Dataset source, Enrichment enrichment, ArrayNode parameters ) {
		...
		tr.setJsonEnrichmentParameters(parameters.toString());
	 ...
	 * or enrichment, throws if it can't make xsl
	 * @return
	 */
	public String getXsl() throws Exception {
		...
		if(( mapping != null ) && mapping.isXslMapping()) {
			...
			try {
				Map<String, String> namespacesUriToPrefix = getParentDataset().getRootHolder().getNamespaces(true);
				Map<String, String> reversePrefixMap = 	namespacesUriToPrefix.entrySet().stream()
					.collect( Collectors.toMap(Map.Entry::getValue, Map.Entry::getKey));
				ArrayNode arrayConfig = (ArrayNode) Jackson.om().readTree(getJsonEnrichmentParameters());
				Optional<String> res = enrichment.getXsl(arrayConfig,  reversePrefixMap );
				return res.get();		
			} catch( Exception e ) {
				log.error( "Enrichment xsl gen failed.", e );
	...
	public String getJsonEnrichmentParameters() {
		return jsonEnrichmentParameters;
	}

	public void setJsonEnrichmentParameters(String jsonEnrichmentParameters) {
		this.jsonEnrichmentParameters = jsonEnrichmentParameters;
	}
}
